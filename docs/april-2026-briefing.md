...
- **[News & Research]({{ site.baseurl }}/docs/news/)** (57 pages) — per-article deep reads of every source cited below. Each page has headline → key takeaways → full notes → what to do → related Playbook pages
...
### Hosted alternative — Claude Code Routines (14 April 2026)

Anthropic shipped **[Routines]({{ site.baseurl }}/docs/news/routines/)** the same week: saved Claude Code configs (prompt + repo + connectors) that run on Anthropic's cloud with three trigger types — **scheduled** (cron-like), **API** (HTTP endpoint), **GitHub webhook**. Research preview. Daily limits: **Pro 5 · Max 15 · Team/Enterprise 25**. Create via `/schedule` or at claude.ai/code.

`/bad` vs Routines, one line each:

| Need | Use |
|:--|:--|
| Dependency-graph-parallel sprint execution, OMC/BMAD tooling in loop | `/bad` (local, worktree-isolated) |
| Nightly triage / weekly audit / webhook-triggered PR analysis, no local infra | Routines (hosted) |
| Regulated vertical (SR 11-7 / EU AI Act) | Neither until per-run audit logging is confirmed |
...
10. **Pilot one Claude Code Routine** on a low-risk scheduled job (nightly dep audit, changelog). Research preview; daily limits are tight on Pro (5), roomier on Team/Enterprise (25). Keep outputs as draft PRs until you've watched a few dozen runs.
11. **Skim the Playbook** (link below) — the full technical detail for every topic above sits in a dedicated page there.
...
### News & Research deep reads (57 articles)
...
*Generated by Claude Opus 4.7 via oh-my-claudecode. The research trail for this briefing spans 69 research items across Medium, Reddit, Forbes, and vendor announcements, consolidated into 57 per-article News & Research pages.*
